...
$runCmd = "docker run -d --name style-sync-agent --restart unless-stopped -e STYLE_ROOT=$StyleRoot -e SUPABASE_URL=https://supabase.lipoout.com -e SUPABASE_SERVICE_ROLE_KEY=$sr -e COMPANY_ID=$CompanyId -e POLL_MS=3000 -e INBOUND_POLL_MS=3000 -e SYNC_EVENT_DRIVEN=1 -e SYNC_POLL_FALLBACK_MS=120000 -e ENTITY_POLL_MS=120000 -e ENTITY_BATCH=15 -v ${StyleRoot}:${StyleRoot}:rw style-sync-agent:0.2.1"
$dockerLines = @(
    "set -e",
    "cd $remoteBase",
