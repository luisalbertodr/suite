...
- Deployment is manual to on-prem servers (see `.cursor/rules/*.mdc`). **Never push to `main` or deploy from a feature branch** (see `.cursor/rules/git-and-deploy-workflow.mdc`). Deploy only after merge to `main`, or when the user explicitly asks post-merge.
