...
export type BuildSegmentsOptions = {
  ...
  /** Fallback Style/Dunasoft codrec de la cita cuando el ítem no trae recurso. */
...
export function buildAppointmentTimeSegments(
  ...
  for (const item of items) {
    ...
    let matched = catalog.length ? matchRecursoForItem(item, catalog, articleHint) : null;
    if (!matched && options.fallbackCodrec) {
      matched = matchRecursoByDunasoftCodrec(options.fallbackCodrec, catalog);
    }
