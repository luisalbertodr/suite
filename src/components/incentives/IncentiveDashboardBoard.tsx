...
function progressPct(current: number, next: number): number {
  if (next <= 0) return 100;
  return Math.min(100, Math.round((current / next) * 100));
}

const TeamMemberCard: React.FC<{ member: IncentiveTeamMember }> = ({ member }) => {
  ...
  return (
    <div className="rounded-lg border border-border/60 p-3">
      ...
      <p className="mt-1 text-[11px] text-muted-foreground tabular-nums">
        {member.remaining_to_next <= 0
          ? `Siguiente tramo: ${member.next_tier_hours} h`
          : isReception
            ? `Faltan ${member.remaining_to_next} para ${member.next_tier_hours} h`
            : `Faltan ${member.remaining_to_next.toLocaleString('es-ES', { maximumFractionDigits: 0 })} € para ${member.next_tier_hours} h`}
      </p>
    </div>
  );
};
...
const IncentiveTeamBoard: React.FC = () => {
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {employees.map((member) => (
              <button
                ...
                type="button"
                onClick={() => setFocusId(member.employee_id)}
                className={`text-left transition ${
                  focused.employee_id === member.employee_id ? 'ring-2 ring-emerald-500 rounded-lg' : ''
                }`}
              >
...
const IncentivePersonalBoard: React.FC = () => {
  ...
  const pct = progressPct(currentValue, Number(data.next_threshold ?? 0));
