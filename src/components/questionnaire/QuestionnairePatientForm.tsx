...
export function QuestionnairePatientForm({
  ...
  const [customerForm, setCustomerForm] = useState({
    ...
    first_session_date: customer.first_session_date?.slice(0, 10) ?? '',
  });
  ...
  const persistDraft = useCallback(async () => {
    ...
    try {
      ...
      await savePatientAnswers(questionnaireId, merged, customer.id, {
        ...
        first_session_date: customerForm.first_session_date || null,
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {returnNote ? (
        <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-md p-3">{returnNote}</p>
      ) : null}
        ...
        {personalReadOnly ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input type="date" value={customerForm.first_session_date} onChange={(e) => setCustomerForm((f) => ({ ...f, first_session_date: e.target.value }))} />
            </div>
            <div>
