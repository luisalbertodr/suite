...
WHERE table_schema='legacy' AND table_name='faclin'
ORDER BY ordinal_position;

SELECT fl.serfac, fl.numfac, fl.codemp, fl.codart, fl.desart, fl.canfac, fl.preven, fl.totlin
FROM legacy.faclin fl
WHERE fl.serfac='A' AND fl.numfac='1448'
LIMIT 10;

-- sales by employee using faclin line totals for july invoices
WITH active_invoices AS (
  SELECT i.id, i.total_amount, m.style_key,
...
line_sales AS (
  SELECT
    ai.id AS invoice_id,
    fl.codemp,
    sum(coalesce(nullif(regexp_replace(btrim(fl.totlin::text), ',', '.', 'g'), '')::numeric, 0)) AS line_amount
  FROM active_invoices ai
  JOIN legacy.faclin fl ON btrim(fl.serfac) = ai.serfac AND btrim(fl.numfac) = ai.numfac
  GROUP BY 1, 2
)
SELECT
  coalesce(ae.name, 'Empleada ' || coalesce(nullif(btrim(ls.codemp),''), '?')) AS name,
  ls.codemp,
