* Crear control_sincro + cola_sincro v2 en la raiz de Style (sin depender de otros PRG).
LPARAMETERS lcDummy
LOCAL lcpath, lclog, lcRoot
lcRoot = ADDBS(JUSTPATH(JUSTPATH(SYS(16,1))))
...
USE IN _ctl
STRTOFILE("OK control_sincro modo=2" + CHR(13), lclog, .T.)
...
IF FILE(lcpath + ".dbf")
   ...
   ZAP
   USE IN _cola
   STRTOFILE("OK cola_sincro ZAP (ya existia)" + CHR(13), lclog, .T.)
ELSE
   ...
   USE
   STRTOFILE("OK cola_sincro CREATE vacia" + CHR(13), lclog, .T.)
ENDIF
QUIT
