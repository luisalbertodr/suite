-- Evita que Style reciba cambios Suite obsoletos tras una edición en Style (VFP/DBF).
-- Sin esto, stylegetreservas puede devolver una cola pendiente y pisar el DBF local.

CREATE OR REPLACE FUNCTION dunasoft.cancel_pending_style_reservas_outbound(p_idplan numeric)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    delivered_at = now(),
    error = 'superseded_by_style_inbound'
...
COMMENT ON FUNCTION dunasoft.cancel_pending_style_reservas_outbound(numeric) IS
  'Marca como entregados los pendientes Suite→Style de un idplan (Style acaba de ser fuente de verdad).';
...
BEGIN
  ...
  -- Sustituir pendientes del mismo idplan por la versión más reciente de Suite.
  PERFORM dunasoft.cancel_pending_style_reservas_outbound(p_idplan);
