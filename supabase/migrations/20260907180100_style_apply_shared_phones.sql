-- Style→Suite apply: teléfonos compartidos + no duplicar tel1=tel2
...
BEGIN
  ...
  -- Si tel1 == tel2 (mismo número), es móvil duplicado: no guardar en phone_home
  IF v_phone_home IS NOT NULL AND v_phone_mobile IS NOT NULL
     AND right(regexp_replace(v_phone_home, '\D', '', 'g'), 9)
       = right(regexp_replace(v_phone_mobile, '\D', '', 'g'), 9) THEN
    v_phone_home := NULL;
  END IF;
  ...
  -- Teléfono compartido permitido: no vincular/fusionar por phone_norm
  ...
  -- Siempre aplicar teléfonos Style (coalesce); unicidad phone_norm ya no bloquea
  v_apply_phones := true;
