-- Refresca los appointment_items de citas Style cuando entra una modificación inbound.
-- Sin este puente, Agenda/Servicios pueden seguir mostrando ítems antiguos aunque
-- agenda_appointments y dunasoft.planart sí se hayan actualizado.
...
BEGIN
  ...
  SELECT
    ...
    row_number() OVER (
      ORDER BY coalesce(nullif(btrim(pa.hora), ''), '99:99'), nullif(btrim(pa.codart), ''), pa.id
    ) - 1 AS sort_order,
    a.id AS article_id,
    1 AS quantity,
