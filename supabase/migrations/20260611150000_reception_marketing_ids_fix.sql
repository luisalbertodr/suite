-- Corrige IDs de empresa (Estética = Delgado Lamas SL, Medicina = María del Mar).
-- La migración anterior usaba UUIDs invertidos respecto a producción.
...
BEGIN
  ...
  IF v_read IS NULL OR v_write IS NULL THEN
    RAISE NOTICE 'Permisos marketing no encontrados';
    RETURN;
  END IF;
  ...
  LOOP
    ...
    INSERT INTO public.user_permission_overrides (
      ...
      (v_user, v_estetica, v_read, NULL, NULL, 'allow', 'reception_marketing_ids_fix'),
      (v_user, v_estetica, v_write, NULL, NULL, 'allow', 'reception_marketing_ids_fix'),
      (v_user, v_medicina, v_read, NULL, NULL, 'deny', 'reception_marketing_ids_fix'),
      (v_user, v_medicina, v_write, NULL, NULL, 'deny', 'reception_marketing_ids_fix');

    INSERT INTO public.user_active_company (user_id, company_id, updated_at)
    VALUES (v_user, v_estetica, now())
