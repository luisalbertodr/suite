-- Cuadro de mandos: filtro opcional por empresa de facturación Suite (Medicina/Estética)
-- según dashboard_resolve_line_billing_company_id (familias/artículos), no Style.

DROP FUNCTION IF EXISTS public.dashboard_command_board_stats(uuid, uuid, date, date);
...
BEGIN
  ...
  -- Líneas con empresa Suite (Medicina/Estética) vía catálogo hub.
  billed_lines AS (
    SELECT
      ai.id AS invoice_id,
      ai.customer_id,
      ...
      ii.description,
      public.dashboard_resolve_line_billing_company_id(
        ii.description,
        v_hub
      ) AS bill_co
    FROM active_invoices ai
  ...
  line_items_typed AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE ar.company_id = p_catalog_company_id
        AND (
          upper(btrim(ar.codigo)) = upper(btrim(substring(sl.description FROM '^([A-Za-z0-9._-]+)\s*[-–—]\s*')))
          OR upper(btrim(ar.descripcion)) = upper(btrim(sl.description))
        )
      ORDER BY
        CASE
          WHEN coalesce(ar.billing_company_id, (
            SELECT af.billing_company_id FROM public.article_families af
            WHERE af.company_id = ar.company_id AND af.name = ar.familia
          )) = p_billing_company_id THEN 0
          ELSE 1
        END,
