-- Alinear importes Style sync con faccab.totfac (cabecera fiscal).
-- Las líneas faclin pueden redondear distinto; el dashboard debe usar totfac.
...
BEGIN
  ...
  IF jsonb_typeof(v_lines) = 'array' AND jsonb_array_length(v_lines) > 0 THEN
    ...
    -- Reconciliar con totfac de faccab (p_total), no con la suma de líneas.
...
COMMENT ON FUNCTION dunasoft.style_facturas_apply_from_style IS
  'Style→Suite faccab. Importes alineados con totfac; líneas escaladas si faclin redondea distinto.';

-- Corregir 2 facturas con issue_date erróneo (2012) pero ejefac 2026.
UPDATE public.invoices i
SET issue_date = '2026-01-31',
    due_date = '2026-01-31',
    updated_at = now()
...
WHERE m.suite_id = i.id
  ...
  AND m.style_key IN (
    '2026/A/8/002067/5d72535b-4e2c-4a5b-9900-e6c5a85f2ce4',
    '2026/A/9/006349/5d72535b-4e2c-4a5b-9900-e6c5a85f2ce4'
  )
  AND i.issue_date < '2020-01-01';
