-- Cuadro de mandos: filtro Medicina/Estética sin llamar dashboard_resolve_line_billing_company_id
-- por cada línea (timeout). Resolución set-based por código / descripción / legacy.
...
BEGIN
  ...
  -- Catálogo hub Style si el usuario puede acceder; si no, la empresa pedida / activa.
  v_catalog := coalesce(nullif(p_catalog_company_id, '00000000-0000-0000-0000-000000000000'::uuid), v_hub);
  IF auth.uid() IS NULL THEN
    v_catalog := coalesce(v_hub, v_catalog);
  ELSIF public.user_can_access_company(v_hub) THEN
    v_catalog := v_hub;
  ELSIF public.user_can_access_company(v_catalog) THEN
    NULL; -- ok
  ELSE
  ...
  WITH active_invoices AS (
    ...
    WHERE i.issue_date BETWEEN p_from_date AND p_to_date
      ...
      AND (
        -- Style = fuente de verdad: solo facturas mapeadas del ejercicio (incl. serie 00 para columna Bonos).
        (v_use_style AND m.company_id = v_hub AND m.style_key LIKE v_year::text || '/%')
        -- Empresas sin Style: facturas de la propia company.
  ...
  -- Catálogo de artículos → empresa fiscal + tipo (una sola pasada).
  article_billing AS (
    SELECT
      a.id,
      upper(btrim(a.codigo)) AS codigo_key,
      nullif(btrim(a.legacy_codart), '') AS legacy_key,
      upper(btrim(a.descripcion)) AS desc_key,
      coalesce(a.billing_company_id, af.billing_company_id, v_catalog) AS bill_co,
      lower(coalesce(nullif(btrim(a.tipo_producto), ''), 'servicio')) AS tipo_producto,
      a.updated_at,
      CASE
        WHEN coalesce(a.billing_company_id, af.billing_company_id) = v_med_billing THEN 0
        ELSE 1
      END AS med_rank
    FROM public.articles a
    LEFT JOIN public.article_families af
      ON af.company_id = a.company_id AND af.name = a.familia
    WHERE a.company_id = v_catalog
  ),
  art_by_desc AS (
    SELECT DISTINCT ON (desc_key) desc_key, bill_co, tipo_producto
    FROM article_billing
    WHERE desc_key IS NOT NULL AND desc_key <> ''
    ORDER BY desc_key, med_rank, updated_at DESC NULLS LAST
  ),
  art_by_code AS (
    SELECT DISTINCT ON (codigo_key) codigo_key, bill_co, tipo_producto
    FROM article_billing
    WHERE codigo_key IS NOT NULL AND codigo_key <> ''
    ORDER BY codigo_key, med_rank, updated_at DESC NULLS LAST
  ),
  art_by_legacy AS (
    SELECT DISTINCT ON (legacy_key) legacy_key, bill_co, tipo_producto
    FROM article_billing
    WHERE legacy_key IS NOT NULL AND legacy_key <> ''
    ORDER BY legacy_key, med_rank, updated_at DESC NULLS LAST
  ),
  -- Líneas con empresa Suite (Medicina/Estética) por JOIN, no por función fila a fila.
  billed_lines AS (
    SELECT
      ai.id AS invoice_id,
      ai.customer_id,
      ...
      coalesce(ii.total_price, 0)::numeric AS line_amount,
      ii.description,
      coalesce(
        CASE
          WHEN nullif(upper(btrim(substring(ii.description FROM '^([A-Za-z0-9._-]+)\s*[-–—]\s*'))), '') IS NOT NULL
          THEN abc.bill_co
        END,
        abd.bill_co,
        CASE
          WHEN (regexp_match(btrim(ii.description), '\[(\d+)\]\s*$'))[1] IS NOT NULL
          THEN abl.bill_co
        END,
        CASE
          WHEN upper(ii.description) ~ '(FOTREJ|FOTORREJ|MANCHA)' THEN v_med_billing
        END,
        v_catalog
      ) AS bill_co,
      coalesce(
        CASE
          WHEN nullif(upper(btrim(substring(ii.description FROM '^([A-Za-z0-9._-]+)\s*[-–—]\s*'))), '') IS NOT NULL
          THEN abc.tipo_producto
        END,
        abd.tipo_producto,
        CASE
          WHEN (regexp_match(btrim(ii.description), '\[(\d+)\]\s*$'))[1] IS NOT NULL
          THEN abl.tipo_producto
        END,
        'servicio'
      ) AS tipo_producto
    FROM active_invoices ai
    INNER JOIN public.invoice_items ii ON ii.invoice_id = ai.id
    LEFT JOIN art_by_code abc
      ON abc.codigo_key = upper(btrim(substring(ii.description FROM '^([A-Za-z0-9._-]+)\s*[-–—]\s*')))
    LEFT JOIN art_by_desc abd
      ON abd.desc_key = upper(btrim(ii.description))
    LEFT JOIN art_by_legacy abl
      ON abl.legacy_key = (regexp_match(btrim(ii.description), '\[(\d+)\]\s*$'))[1]
  ),
  scoped_lines AS (
    SELECT *
    FROM billed_lines bl
    WHERE p_billing_company_id IS NULL
       OR bl.bill_co = p_billing_company_id
  ),
  scoped_invoices AS (
    SELECT DISTINCT ON (invoice_id)
      ...
      CASE WHEN serfac = '00' THEN 'bonos' ELSE 'other' END AS bucket
    FROM scoped_lines
    ORDER BY invoice_id
  ),
  ticket_stats AS (
    SELECT
      (
        SELECT count(*)::bigint FROM (
          SELECT 1 FROM invoice_serfac WHERE p_billing_company_id IS NULL
          UNION ALL
          SELECT 1 FROM scoped_invoices WHERE p_billing_company_id IS NOT NULL
        ) t
      ) AS tickets_total,
      (
        SELECT count(*)::bigint FROM (
          SELECT 1 FROM invoice_serfac WHERE p_billing_company_id IS NULL AND bucket = 'bonos'
          UNION ALL
          SELECT 1 FROM scoped_invoices WHERE p_billing_company_id IS NOT NULL AND bucket = 'bonos'
        ) t
      ) AS tickets_bonos,
      (
        SELECT count(*)::bigint FROM (
          SELECT 1 FROM invoice_serfac WHERE p_billing_company_id IS NULL AND bucket = 'other'
          UNION ALL
          SELECT 1 FROM scoped_invoices WHERE p_billing_company_id IS NOT NULL AND bucket = 'other'
        ) t
      ) AS tickets_other,
      CASE
        WHEN p_billing_company_id IS NULL THEN
          round(coalesce((SELECT sum(total_amount) FROM invoice_serfac), 0)::numeric, 2)
        ELSE
          round(coalesce((SELECT sum(line_amount) FROM scoped_lines), 0)::numeric, 2)
      END AS invoiced_total,
      CASE
        WHEN p_billing_company_id IS NULL THEN
          round(coalesce((SELECT sum(total_amount) FROM invoice_serfac WHERE bucket = 'bonos'), 0)::numeric, 2)
        ELSE
          round(coalesce((SELECT sum(line_amount) FROM scoped_lines WHERE serfac = '00'), 0)::numeric, 2)
      END AS invoiced_bonos,
      CASE
        WHEN p_billing_company_id IS NULL THEN
          round(coalesce((SELECT sum(total_amount) FROM invoice_serfac WHERE bucket = 'other'), 0)::numeric, 2)
        ELSE
          round(coalesce((SELECT sum(line_amount) FROM scoped_lines WHERE serfac IS DISTINCT FROM '00'), 0)::numeric, 2)
      END AS invoiced_other,
      CASE
        WHEN p_billing_company_id IS NULL THEN
          round(coalesce((SELECT sum(greatest(total_amount - coalesce(amount_paid, 0), 0)) FROM invoice_serfac), 0)::numeric, 2)
        ELSE
          round(coalesce((SELECT sum(greatest(total_amount - coalesce(amount_paid, 0), 0)) FROM scoped_invoices), 0)::numeric, 2)
      END AS debts_total
  ),
  bono_stats AS (
    SELECT
      CASE WHEN p_billing_company_id IS NULL THEN count(*)::bigint ELSE 0::bigint END AS extra_tickets,
      CASE
        WHEN p_billing_company_id IS NULL THEN
  ...
  line_stats AS (
    SELECT
      round(coalesce(sum(line_amount) FILTER (
    ...
    FROM scoped_lines
    WHERE serfac IS DISTINCT FROM '00'
  ),
  faclin_amount AS (
    SELECT
  ...
  faclin_sales AS (
    ...
    WHERE p_billing_company_id IS NULL
       OR EXISTS (SELECT 1 FROM scoped_invoices si WHERE si.id = ai.id)
  ...
  employee_sales_lines AS (
    ...
    UNION ALL
    SELECT ai.id, '' AS codemp,
      CASE
        WHEN p_billing_company_id IS NULL THEN ai.total_amount
        ELSE coalesce((
          SELECT sum(sl.line_amount) FROM scoped_lines sl WHERE sl.invoice_id = ai.id
        ), 0)
      END AS amount
    FROM active_invoices ai
  ...
  top_article AS (
    SELECT
      coalesce(nullif(btrim(sl.description), ''), 'Sin descripción') AS name,
      round(sum(sl.line_amount)::numeric, 2) AS amount
    FROM scoped_lines sl
    WHERE sl.serfac IS DISTINCT FROM '00'
    GROUP BY 1
  ...
  top_customer AS (
    SELECT
      ...
      round(sum(sl.line_amount)::numeric, 2) AS amount
    FROM scoped_lines sl
    INNER JOIN public.customers c ON c.id = sl.customer_id
    WHERE sl.serfac IS DISTINCT FROM '00'
    GROUP BY 1
    ORDER BY amount DESC
    LIMIT 1
  ...
  period_clients AS (
    SELECT count(DISTINCT sl.customer_id)::bigint AS total
    FROM scoped_lines sl
...
GRANT EXECUTE ON FUNCTION public.dashboard_command_board_stats(uuid, uuid, date, date, uuid)
