-- Número de factura Style incluye ejercicio: A-2012-382 (evita colisión multi-año).
...
BEGIN
  ...
  FOR v_bucket IN SELECT * FROM tmp_style_inv_buckets WHERE total <> 0 LOOP
    ...
    v_number := coalesce(nullif(v_serie, ''), 'A') || '-' || v_fiscal_year::text || '-' || v_numfac;
    v_legacy_number := coalesce(nullif(v_serie, ''), 'A') || '-' || v_numfac;
    IF v_groups > 1 AND v_prefix IS NOT NULL THEN
      v_number := v_number || '-' || v_prefix;
      v_legacy_number := v_legacy_number || '-' || v_prefix;
    END IF;
    ...
    IF v_invoice_id IS NULL THEN
      ...
      WHERE company_id = v_bucket.billing_company_id
        AND number IN (v_number, v_legacy_number)
        AND extract(year FROM issue_date) = v_fiscal_year
      ORDER BY CASE WHEN number = v_number THEN 0 ELSE 1 END
      LIMIT 1;
    END IF;
