-- Deuda pendiente de facturas v3:
-- 1) Evita doble conteo FAC "sin cita" / "automática" cuando ya existe LEG rebuild.
-- 2) Respeta amount_paid de invoices.
-- 3) Para clientes legacy, la deuda Style (legacy.clientes.deuda) es la fuente de verdad
--    de saldos históricos; se suman solo facturas Suite no-legacy encima.
...
AS $$
  ...
  inv AS (
    ...
    WHERE i.company_id = p_company_id
      ...
      -- Duplicados: automática / sin cita si hay rebuild FACCAB el mismo día
      AND NOT (
        (
      ...
      -- Duplicado exacto sin cita ↔ rebuild por numfac
      AND NOT (
  ...
  SELECT COALESCE(
    ROUND(
      (
        ...
        + CASE
            -- Cliente legacy: Style lleva el saldo histórico
            WHEN (SELECT legacy_codcli FROM cust) IS NOT NULL
              AND (SELECT deuda FROM style_deuda) IS NOT NULL
...
COMMENT ON FUNCTION public.customer_pending_invoice_debt(uuid, uuid) IS
  'Deuda pendiente: facturas Suite nativas + legacy.clientes.deuda (o facturas legacy deduplicadas si no hay codcli).';
