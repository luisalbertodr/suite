-- Corrige el parseo de obsoleto en la sincronización de empleados Style.
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      (
        nullif(btrim(coalesce(e.fecbaja::text, '')), '') IS NULL
        AND upper(coalesce(nullif(btrim(coalesce(e.obsoleto::text, '')), ''), 'NO')) NOT IN ('S', 'SI', '1', 'TRUE', 'T', 'Y', 'YES')
      ) AS style_active
