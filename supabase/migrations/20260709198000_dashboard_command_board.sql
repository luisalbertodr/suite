-- Cuadro de mandos Style: estadísticas por rango de fechas (ventas, clientes, reservas, caja).

CREATE OR REPLACE FUNCTION dunasoft.plan_slot_minutes(p_horini text, p_horfin text)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN coalesce(btrim(p_horini), '') = '' OR coalesce(btrim(p_horfin), '') = '' THEN 0::numeric
    ELSE greatest(
      0::numeric,
      extract(
        epoch FROM (
          CASE
            WHEN to_timestamp(p_horfin, 'HH24:MI') < to_timestamp(p_horini, 'HH24:MI')
              THEN to_timestamp(p_horfin, 'HH24:MI') + interval '1 day'
            ELSE to_timestamp(p_horfin, 'HH24:MI')
          END - to_timestamp(p_horini, 'HH24:MI')
        )
      ) / 60.0
    )
  END;
...
BEGIN
  ...
  WITH active_invoices AS (
    SELECT
      ...
      nullif(split_part(m.style_key, '/', 5), '') AS codemp,
      m.style_key
  ...
  bono_sales AS (
    ...
    UNION ALL
    SELECT cv.id, cv.paid_amount AS amount, cv.customer_id, coalesce(a.name, 'Bono') AS label
  ...
  line_stats AS (
    SELECT
      round(coalesce(sum(ii.total_price) FILTER (
        WHERE coalesce(lower(a.tipo_producto), 'servicio') IN ('servicio', 'service')
      ), 0)::numeric, 2) AS services_amount,
      count(*) FILTER (
        WHERE coalesce(lower(a.tipo_producto), 'servicio') IN ('servicio', 'service')
      )::bigint AS services_count,
      round(coalesce(sum(ii.total_price) FILTER (
        WHERE coalesce(lower(a.tipo_producto), 'servicio') IN ('producto', 'product', 'textil', 'calzado', 'standard')
  ...
  top_employee AS (
    SELECT
      coalesce(ae.name, 'Empleada ' || coalesce(ai.codemp, '?')) AS name,
    ...
    LEFT JOIN public.agenda_employees ae
     ...
     AND coalesce(nullif(ltrim(btrim(coalesce(ae.dunasoft_codemp, '')), '0'), ''), '0')
       = coalesce(nullif(ltrim(btrim(coalesce(ai.codemp, '')), '0'), ''), '0')
    WHERE ai.codemp IS NOT NULL
    GROUP BY 1
  ...
  merged AS (
    SELECT
      ...
      ls.products_amount,
      ls.products_count,
      te.name AS top_employee_name,
      te.amount AS top_employee_amount,
    ...
    LEFT JOIN top_employee te ON true
  ...
  SELECT jsonb_build_object(
    ...
    'sales', jsonb_build_object(
      ...
      'debts', m.debts_total,
      'topEmployee', jsonb_build_object('name', coalesce(m.top_employee_name, '—'), 'amount', coalesce(m.top_employee_amount, 0)),
...
GRANT EXECUTE ON FUNCTION public.dashboard_command_board_stats(uuid, uuid, date, date) TO authenticated, service_role;

COMMENT ON FUNCTION public.dashboard_command_board_stats(uuid, uuid, date, date) IS
  'Cuadro de mandos Style: ventas, clientes, reservas, caja y tops para un rango de fechas.';
