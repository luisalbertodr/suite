-- Style push: no sobrescribir codemp/codcli reales con '0' (snapshot vacío en cola).

CREATE OR REPLACE FUNCTION dunasoft.style_code_or_keep(p_new text, p_old text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(
    nullif(nullif(btrim(p_new), ''), '0'),
    nullif(nullif(btrim(p_old), ''), '0'),
    '0'
  );
$$;
