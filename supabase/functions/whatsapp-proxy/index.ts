...
type GroupJoinRequestsBody = {
  action:
    | 'groups.join_requests.list'
    | 'groups.join_requests.approve'
    | 'groups.join_requests.reject'
    | 'groups.membership_approval.get'
    | 'groups.membership_approval.set';
  chat_id: string;
...
serve(async (req) => {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'messages.edit': {
        if (provider !== 'waha') {
          return err('Editar mensajes solo está disponible con WAHA');
        }
      ...
      case 'groups.join_requests.list':
      case 'groups.join_requests.approve':
      case 'groups.join_requests.reject':
      case 'groups.membership_approval.get':
      case 'groups.membership_approval.set': {
        if (provider !== 'waha') {
          return err('La gestión de grupos solo está disponible con WAHA');
        }
        const groupBody = body as GroupJoinRequestsBody;
        const groupId = String(groupBody.chat_id ?? '').trim();
        if (!groupId || !isGroupJid(groupId)) {
          return err('Falta chat_id de grupo válido');
        }
        const groupBase =
          `/api/${encodeURIComponent(sessionName)}/groups/${encodeURIComponent(groupId)}`;

        if (groupBody.action === 'groups.membership_approval.get') {
          const setting = await wahaJson<{ newMembersApprovalRequired?: boolean }>(
            cfg,
            `${groupBase}/settings/security/membership-approval`,
          );
          return json({
            ok: true,
            chat_id: groupId,
            new_members_approval_required: !!setting?.newMembersApprovalRequired,
          });
        }

        if (groupBody.action === 'groups.membership_approval.set') {
          if (typeof groupBody.new_members_approval_required !== 'boolean') {
            return err('Falta new_members_approval_required');
          }
          await wahaJson(cfg, `${groupBase}/settings/security/membership-approval`, {
            method: 'PUT',
            body: JSON.stringify({
              newMembersApprovalRequired: groupBody.new_members_approval_required,
            }),
          });
          return json({
            ok: true,
            chat_id: groupId,
            new_members_approval_required: groupBody.new_members_approval_required,
          });
        }

        if (groupBody.action === 'groups.join_requests.list') {
          const requests = await wahaJson<unknown>(
            cfg,
            `${groupBase}/participants/join-requests`,
          );
          const list = Array.isArray(requests) ? requests : [];
          return json({ ok: true, chat_id: groupId, requests: list });
        }

        const ids = (groupBody.participant_ids ?? [])
          .map((id) => String(id ?? '').trim())
          .filter(Boolean);
        if (ids.length === 0) return err('Falta participant_ids');
        const pathAction =
          groupBody.action === 'groups.join_requests.approve' ? 'approve' : 'reject';
        const result = await wahaJson<unknown>(
          cfg,
          `${groupBase}/participants/join-requests/${pathAction}`,
          {
            method: 'POST',
            body: JSON.stringify({
        ...
        return json({
          ...
          chat_id: groupId,
          action: pathAction,
          result: Array.isArray(result) ? result : result ?? null,
        });
      }
